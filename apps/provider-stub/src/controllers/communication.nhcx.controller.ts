...
export class CommunicationNHCXController {
  ...
  async handleCommunicationOnRequest(req: Request, res: Response): Promise<void> {
    try {
      ...
      protectedHeaders['x-hcx-sender_code'] = this.nhcxService.config.providerCode.trim();
      protectedHeaders['x-hcx-recipient_code'] = this.nhcxService.config.payerCode.trim();
